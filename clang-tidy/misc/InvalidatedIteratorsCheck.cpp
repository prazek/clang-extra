...
void InvalidatedIteratorsCheck::check(const MatchFinder::MatchResult &Result) {
  ...
  for (const auto &BlockElem : *Block) {
    ...
    auto Results = match(PushBackMatcher,
                         *(BlockElem.getAs<CFGStmt>()->getStmt()), *Context);
    for (const auto &Match : Results) {
      const auto *PushBackUse =
          Match.getNodeAs<CXXMemberCallExpr>("PushBackCall");
      if (Sequence->potentiallyAfter(PushBackUse, RefDeclStmt) &&
          Sequence->potentiallyAfter(RefUse, PushBackUse)) {
        diag(RefUse->getLocation(), "%0 might be invalidated before the access")
            << RefDecl;
        diag(PushBackUse->getExprLoc(), "possible place of invalidation",
             DiagnosticIDs::Note);
        return;
      }
    }
