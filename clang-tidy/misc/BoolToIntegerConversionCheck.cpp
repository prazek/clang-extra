...
void BoolToIntegerConversionCheck::registerMatchers(MatchFinder *Finder) {
  ...
  auto comparingWithBool = hasParent(binaryOperator(hasLHS(boolOperand), hasRHS(boolOperand)));


  auto isInReturnStmt = hasParent(returnStmt());
  auto forTheSameFunction = forFunction(equalsBoundNode("function"));
  auto returnNotBool = returnStmt(forTheSameFunction, unless(has(expr(hasType(booleanType())))));
  auto allReturnsBool = hasBody(unless(has(returnNotBool)));
  ...
  Finder->addMatcher(decl(functionDecl().bind("function"),
...
void BoolToIntegerConversionCheck::changeFunctionReturnType(
  ...
  diag(FD.getLocation(),
       "function has return type %0 but returns only bools")
      << FD.getReturnType();
